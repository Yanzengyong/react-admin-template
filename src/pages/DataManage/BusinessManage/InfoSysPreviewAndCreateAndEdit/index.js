...
import InfoSystemPreviewAndCreateAndEditService from '@/componentsService/InfoSystemPreviewAndCreateAndEditService'
...
class InfoSysPreviewAndCreateAndEdit extends React.Component {
	state = {
		pageType: 'create',
		initFieldUuid: '',
		displayAdvance: false,
		businessCatalogChangeDisable: true
	}
	...
	static getDerivedStateFromProps (props, state) {
		...
		if (Item && Item.type === 'create') {
			return {
				pageType: 'create',
				businessCatalogChangeDisable: _search !== ''
			}
		} else if (Item && Item.type === 'edit') {
			if (currentInfoSysUuid !== prevUuid) {
				return {
					pageType: 'edit',
					initFieldUuid: currentInfoSysUuid,
					businessCatalogChangeDisable: _search !== ''
				}
			}
	...
	render () {
		...
		const navInfo = [
			...
			{ label: '访问信息', value: 'visitInfo' },
			{ label: '部署信息', value: 'releaseInfo' },
			{ label: '数据信息', value: 'dataInfo' },
		...
		const { pageType, initFieldUuid, displayAdvance, businessCatalogChangeDisable } = this.state
		...
		return (
			<InfoLayout
				...
				pageType={pageType}
			>
				<InfoSystemPreviewAndCreateAndEditService
					key={initFieldUuid}
					pageType={pageType}
					initFieldUuid={initFieldUuid}
					showAdvance={this.showAdvance}
					displayAdvance={displayAdvance}
					businessCatalogChangeDisable={businessCatalogChangeDisable}
					onBack={this.onBack}
					{...this.props}
				/>
			</InfoLayout>
...
)(InfoSysPreviewAndCreateAndEdit)
