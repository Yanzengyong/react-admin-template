...
import { Form, Upload, Button } from '@alifd/next'
...
export default class UploadFormItem extends React.Component {
  ...
  onDeleteFile = (fileIndex) => {
  	let tempArr = this.props.currentFileList
  ...
  render () {
  	const { pageType, currentFileList, uploadLimit, newFileList } = this.props

		// const currentFileLength = currentFileList ? currentFileList.length : 0
  	const uploadBoxLimit = uploadLimit - currentFileList.length - newFileList.length

  	const resetBtn = (
  		<Button
  			text
  			type="primary"
  			style={{ marginLeft: 10, display: pageType === 'preview' ? 'none' : '' }}
  	...
  	return (
  		<div>
              ...
              待上传文件：
  						<font color="red">
                （共计可添加{uploadLimit}个文件，已上传{currentFileList.length}
...
UploadFormItem.propTypes = {
	...
	currentFileList: PropTypes.array, // 当前已上传文件列表
