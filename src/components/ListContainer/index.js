...
import { Button, Search, Message, Pagination } from '@alifd/next'
import './index.scss'
...
export default class PageHead extends React.Component {
	...
	static propTypes = {
		...
		handleBatchImport: PropTypes.func,
	...
	static defaultProps = {
		...
		batchImport: false, // 是否提供批量导入按钮
		qualityReport: false, // 是否提供数据质量报告导出
		qualityRule: false, // 是否提供数据质量规则模板设置
		selectedNum: 0, // 已经选择了x个行
		handleSearch: () => { }, // 右标题部分的搜索
		createHandle: () => { }, // 创建的按钮
		deleteAllHandle: () => { }, // 删除的按钮
		handleBatchImport: () => { }, // 批量导入的按钮
		handleExportQualityReport: ()=>{}, // 数据质量报告导出的按钮
		handleManageQualityRule: () => {}, // 数据质量规则模板设置的按钮
		rightNode: null, // 右边自定义的node
		leftNode: null, // 左边自定义的node
		size: 'medium', // 分页组件大小
		type: 'normal', // 分页组件类型
		shape: 'normal', // 前进后退按钮样式 'normal', 'arrow-only', 'arrow-prev-only', 'no-border'
	...
	render () {
		const {
			...
			deleteAllHandle,
			handleBatchImport,
			handleExportQualityReport,
			handleManageQualityRule,
			rightNode,
			leftNode,
			size,
			type,
		...
		return (
			<div className="listContainer">
				<div className="listContainer_utilbox">
					{
						leftNode ? leftNode : (
							<div className="listContainer_utilbox_left">
								<Search
									shape="simple"
									className='listContainer_util_input'
					...
					{rightNode ? <div className="listContainer_utilbox_right">{rightNode}</div> : (
						<div className="listContainer_utilbox_right">
							...
							>{leftBtnText ? leftBtnText : '+ 新建'}</Button>
							<Button
								style={{ marginRight: '10px' }}
								className='listContainer_utilbox_right_btn'
								type="primary"
							...
							{qualityRule ? (<Button
								...
								className='listContainer_utilbox_right_btn'
								type="secondary"
								onClick={handleManageQualityRule}
							>规则模板设置</Button>) : ''}

							{qualityReport ? (<Button
								style={{ marginRight: '10px' }}
								className='listContainer_utilbox_right_btn'
								type="secondary"
				...
				{!multipleSelect ? <Message className='listContainer_utilbox_notice' title={'请选择需要操作的对象'} type='notice' shape='inline' /> : (
					<Message className='listContainer_utilbox_notice' title={`已经选择了${selectedNum}项`} type='notice' shape='inline' />
				)}
