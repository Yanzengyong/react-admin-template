...
export default class ViewsPreviewAndCreateAndEditService extends React.Component {
	...
	componentDidMount () {
		...
		if (initData) {
			...
			if (this.props.pageType === 'edit' || this.props.pageType === 'preview') {
				...
				this.field.setValue('informationSystemsUuid', getQueryItemValue(this.props.location.search, 'title'))
			}
		}
	}
	...
	initFieldList = async (uuid) => {
		...
		if (response) {
			if (response.code === 10000) {
				...
				this.field.setValue('informationSystemsUuid', getQueryItemValue(this.props.location.search, 'informationSystemsName'))

				// 因fileResult内容不存在时结果为null而非[],故需进行判断处理
				const fileList = response.result.fileResult ? response.result.fileResult : []
				this.setState({
		...
		this.field.validate((errors, values) => {
			...
			if (newArrResult === 'error') return
				...
				this.setState({ viewInfoData: newArrResult }, () => {
					if (this.props.pageType === 'create') this.addView(values, this.state.viewInfoData)
					if (this.props.pageType === 'edit') this.updateView(values, this.state.viewInfoData)
				})
			}
		})
	}
	...
	addView = async (values, viewInfoArr) => {
		...
		// 2. 处理上传参数
		let param = {
			informationSystemsUuid: getQueryItemValue(this.props.location.search, 'informationSystemsUuid'),
			name: values.name,
			memo: values.memo,
